...
export const handleRankUpdate = async (userId) => {
  ...
  if (rank === null) {
    ...
    if (calculatedRank === null || calculatedRank < user.rank) {
      rank = hasNewOrder ? Math.max(0, user.rank - 1) : user.rank;
        ...
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 24px; border: 1px solid #ddd; border-radius: 8px;">
          ...
          ${
            ...
            <div style="margin-top: 24px; padding: 16px; background-color: #f9f9f9; border-left: 4px solid #4CAF50;">
              ...
              <p><strong>Hạn dùng:</strong> đến ${dayjs(voucher.endDate).format(
                "DD/MM/YYYY"
              )}</p>
            </div>
  ...
  return { rank, spendingScore, totalSpending, orders };
};

export const getCustomerRank = async (req, res) => {
  try {
    const userId = req.params.id;
    const result = await handleRankUpdate(userId);
    if (!result)
      return res.status(404).json({ message: "Không tìm thấy user" });

    const { rank, spendingScore, totalSpending, orders } = result;
