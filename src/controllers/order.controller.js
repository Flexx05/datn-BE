      ...
      for (const item of items) {
        ...
        if (!product) {
          return res
            .status(404)
            .json({
              error: `Không tìm thấy sản phẩm chứa biến thể ${item.variationId}`,
            });
        }
        ...
        if (!variation) {
          return res
            .status(404)
            .json({ error: `Không tìm thấy biến thể ${item.variationId}` });
        }
        ...
        if (!variation.isActive) {
          return res
            .status(400)
            .json({
              error: `Biến thể ${variation._id} của sản phẩm ${product.name} không khả dụng`,
            });
        }
      ...
      for (const code of voucherCode) {
        ...
        if (
          ...
          return res
            .status(400)
            .json({
              error: `Voucher ${voucher.code} không hợp lệ hoặc đã hết hạn`,
            });
        }
        ...
        if (voucher.used >= voucher.quantity) {
          return res
            .status(400)
            .json({ error: `Voucher ${voucher.code} đã hết lượt sử dụng` });
        }
